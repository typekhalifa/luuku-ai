...
if (success?.pattern !== "SUCCESS_PATTERN") throw new Error("AA/AB demo expected a success pattern.");
if (repeatedFailure?.pattern !== "REPEATED_FAILURE") throw new Error("AA/AB demo expected repeated failure learning.");
if (repeatedFailure.failedOccurrences !== 2) throw new Error("AA/AB demo expected two provider-sync failures.");
if (repeatedFailure.lesson !== "Provider sync needs bounded retry handling.") throw new Error("AA/AB demo expected the first recorded lesson to remain inspectable.");
