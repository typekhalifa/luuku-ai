...
export class ExecutiveObjectiveEngine {
    ...
    async assess(objective: ExecutiveObjectiveRecord, state: ExecutiveState): Promise<ObjectiveAssessment> {
        if (objective.status === "COMPLETED") return { objectiveId: objective.id, status: objective.status, progress: objective.progress, attentionRequired: false, reason: "Objective is already completed." };
        if (objective.status === "PAUSED") return { objectiveId: objective.id, status: objective.status, progress: objective.progress, attentionRequired: false, reason: "Objective is paused." };
        if (state.failed > 0) return { objectiveId: objective.id, status: objective.status, progress: objective.progress, attentionRequired: true, reason: "Objective remains active while failed work requires executive attention." };
        if (state.waitingApproval > 0) return { objectiveId: objective.id, status: objective.status, progress: objective.progress, attentionRequired: true, reason: "Objective remains active while founder approval is pending." };
        return { objectiveId: objective.id, status: objective.status, progress: objective.progress, attentionRequired: true, reason: "Objective is active and requires the executive to determine its next useful work." };
    }
}
