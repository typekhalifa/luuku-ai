import assert from "node:assert/strict";

import { Priority } from "../../task/priority";
import { TaskStatus } from "../../task/task-status";
import { TaskType } from "../../task/task-type";
import { Plan } from "../../planner/plan";
import { WorkflowCompiler } from "../workflow-compiler";
import { WorkflowEngine } from "../workflow-engine";
import { WorkflowStatus } from "../workflow-status";

const now = new Date();

const plan: Plan = {
    id: "v6-plan-onboard-company-x",
    goal: "Prepare Company X for founder-approved onboarding.",
    tasks: [
        {
            id: "research-company",
            title: "Research Company X",
            description: "Research the prospect and produce an actionable company brief.",
            type: TaskType.RESEARCH,
            priority: Priority.HIGH,
            status: TaskStatus.PENDING,
            input: { company: "Company X" },
            metadata: {
                agentId: "research",
                capability: "research.company",
            },
            createdAt: now,
            updatedAt: now,
        },
        {
            id: "prepare-proposal",
            title: "Prepare Proposal",
            description: "Create a proposal using the completed research brief.",
            type: TaskType.PROPOSAL,
            priority: Priority.HIGH,
            status: TaskStatus.PENDING,
            input: { company: "Company X" },
            metadata: {
                agentId: "sales",
                capability: "sales.proposal",
                dependsOn: ["research-company"],
            },
            createdAt: now,
            updatedAt: now,
        },
        {
            id: "send-proposal",
            title: "Send Proposal",
            description: "Send the approved proposal to Company X.",
            type: TaskType.OUTREACH,
            priority: Priority.HIGH,
            status: TaskStatus.PENDING,
            input: { company: "Company X" },
            metadata: {
                agentId: "sales",
                capability: "email.send",
                dependsOn: ["prepare-proposal"],
                requiresApproval: true,
            },
            createdAt: now,
            updatedAt: now,
        },
    ],
    metadata: {
        source: "v6-plan-to-workflow-demo",
    },
    createdAt: now,
};

const compiler = new WorkflowCompiler();
const engine = new WorkflowEngine();
const workflow = compiler.compile(plan);

console.log("");
console.log("========================================");
console.log("     V6 PLAN → WORKFLOW DEMO");
console.log("========================================");
console.log("");
console.log(`Plan goal: ${plan.goal}`);
console.log(`Workflow : ${workflow.id}`);
console.log(`Status   : ${workflow.status}`);
console.log(`Steps    : ${workflow.steps.length}`);
console.log(`Approval : ${workflow.requiresFounderApproval}`);

assert.equal(workflow.status, WorkflowStatus.READY);
assert.equal(workflow.requiresFounderApproval, true);
assert.deepEqual(workflow.steps.map((step) => step.id), [
    "research-company",
    "prepare-proposal",
    "send-proposal",
]);
assert.deepEqual(workflow.steps[1].dependsOn, ["research-company"]);
assert.deepEqual(workflow.steps[2].dependsOn, ["prepare-proposal"]);
assert.equal(workflow.steps[2].requiresApproval, true);

let decision = engine.evaluate(workflow);
console.log("");
console.log("Initial workflow decision:");
console.log(`  Runnable : ${decision.runnableStepIds.join(", ") || "none"}`);
console.log(`  Waiting  : ${decision.waitingStepIds.join(", ") || "none"}`);
console.log(`  Blocked  : ${decision.blockedStepIds.join(", ") || "none"}`);

assert.deepEqual(decision.runnableStepIds, ["research-company"]);
assert.deepEqual(decision.waitingStepIds, ["prepare-proposal", "send-proposal"]);
assert.deepEqual(decision.blockedStepIds, []);

workflow.steps[0].status = "COMPLETED";
workflow.steps[0].output = { company: "Company X", qualified: true };

decision = engine.evaluate(workflow);
assert.deepEqual(decision.runnableStepIds, ["prepare-proposal"]);

workflow.steps[1].status = "COMPLETED";
workflow.steps[1].output = { proposalReady: true };
workflow.status = WorkflowStatus.AWAITING_APPROVAL;
workflow.updatedAt = new Date();

decision = engine.evaluate(workflow);
console.log("");
console.log("After preparation:");
console.log(`  Runnable : ${decision.runnableStepIds.join(", ") || "none"}`);
console.log(`  Waiting  : ${decision.waitingStepIds.join(", ") || "none"}`);
console.log(`  Approval : ${decision.requiresApproval}`);

assert.deepEqual(decision.runnableStepIds, []);
assert.deepEqual(decision.waitingStepIds, []);
assert.deepEqual(decision.blockedStepIds, ["send-proposal"]);
assert.equal(decision.requiresApproval, true);

workflow.status = WorkflowStatus.READY;
workflow.approvedAt = new Date();
workflow.updatedAt = new Date();

decision = engine.evaluate(workflow);
console.log("");
console.log("After founder approval:");
console.log(`  Runnable : ${decision.runnableStepIds.join(", ") || "none"}`);
console.log(`  Waiting  : ${decision.waitingStepIds.join(", ") || "none"}`);
console.log(`  Blocked  : ${decision.blockedStepIds.join(", ") || "none"}`);

assert.deepEqual(decision.runnableStepIds, ["send-proposal"]);
assert.deepEqual(decision.waitingStepIds, []);
assert.deepEqual(decision.blockedStepIds, []);
assert.equal(decision.requiresApproval, false);

console.log("");
console.log("✓ Plan compiled into a dependency-aware workflow.");
console.log("✓ Workflow preserved agent, capability, dependency, and approval metadata.");
console.log("✓ Workflow engine evaluated the compiled graph correctly.");
console.log("✓ No agent or external provider was executed.");
console.log("");
