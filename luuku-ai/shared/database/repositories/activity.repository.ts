...
export class ActivityRepository {
    ...
    async findIncomplete(limit?: number): Promise<Activity[]> {
        const activities = await prisma.activity.findMany({
    ...
    async findOverdue(limit?: number): Promise<Activity[]> {
        const activities = await prisma.activity.findMany({
            where: {
                completed: false,
                dueAt: { lt: new Date() }
    ...
    async updateOutcome(id: string, outcome: string, description?: string): Promise<Activity> {
